...
def plot(x,y,prediction):
    ...
    if np.sum(target)>0:
        for i in np.where(target>0)[0]:
            ...
            width = 48
            height = 48
            plt.imshow(x[i].reshape(width,height))
            print("Confidence_max is {}, Predicted is {}, Target is {}".format(np.max(prediction[i]), names[np.argmax(prediction[i])],names[np.argmax(y[i])]))
            plt.show()
    return
...
def main():
    ...
    wbb =  FaceRec("./wbb.jpg")
    inputs_test = np.concatenate((inputs_test, wbb), axis = 0)
    wbb_label = np.array([0,0,0,1,0,0,0]).reshape((1,7))
    target_test = np.concatenate((target_test, wbb_label), axis = 0)
